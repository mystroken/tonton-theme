...
var ExpandTransition = Barba.BaseTransition.extend({
	start: function () {
		Promise
			.all([this.newContainerLoading, this.beforeLeave()])
	...
	beforeLeave: function () {
		...
		// ===========================================================================
			...
			TweenMax.set($pathOverlay0, { fill: "#aaa" })
			TweenMax.set($pathOverlay1, { fill: "#fff" })
			TweenMax.delayedCall(1.5, () => {
		...
		TweenMax.to('body', 0.1, { overflowY: 'hidden', onComplete: () => { deferred.resolve(); } })
...
let archivedBase = Barba.BaseView.extend({
	...
	onLeave: function () {
		TweenMax.to('.modal-zoom, #main-archived', .5, { autoAlpha: 0 })
		archived.destroy()
	}
});
archivedBase.init();
...
let notFoundBase = Barba.BaseView.extend({
	...
	onEnterCompleted: function () {
		...
		TweenMax.set($pathOverlay1, { fill: "#000" })
		TweenMax.staggerFrom('.main-404__hold > *', 1.6, { delay: 2.15, y: 80, autoAlpha: 0, ease: Power3.easeOut }, .1)
	}
});
notFoundBase.init();
